...
def print_model_info(ov_model:ov.Model):
    ...
    for op in ov_model.get_ordered_ops():
        try:
            ...
            print(f"==\t Found op: '{op.get_friendly_name()}' with type '{op.get_type_name()}'. Input shape: {input_shape}, Output shape: {output_shape}")
        except Exception as e:
            print(f"==\t Found op: '{op.get_friendly_name()}' with type '{op.get_type_name()}'. Error getting shapes: {e}")
